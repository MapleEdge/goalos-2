...
export async function generateRecommendations(): Promise<ReasoningOutput> {
  ...
  const missingPrerequisites = computeMissingPrerequisites(goals, prerequisites)
...
type PrereqWithRelations = Awaited<
  ReturnType<
    typeof prisma.prerequisite.findMany<{
...
type StakeholderWithEvidence = Awaited<
  ReturnType<
    typeof prisma.stakeholder.findMany<{ include: { evidence: true } }>
  >
>[number]

type EvidenceWithRelations = Awaited<
  ReturnType<
    typeof prisma.evidence.findMany<{
      include: { stakeholder: true; prerequisite: true }
    }>
  >
>[number]
...
function computeHighestLeverageActions(
  ...
  for (const goal of goals) {
    ...
    const pendingActions = actions.filter(
      (a) =>
        a.goalId === goal.id && a.status !== 'DONE' && a.status !== 'SKIPPED'
    )
...
function computeImportantRelationships(
  ...
  return stakeholders
    ...
    .sort((a, b) => b.linkedGoals.length - a.linkedGoals.length)
    .slice(0, 10)
}
  ...
  return goals.map((goal) => {
    ...
    if (totalPrereqs === 0) {
      return {
        ...
        supportingEvidence: [],
        missingItems: [
          'No prerequisites defined — add prerequisites to track readiness',
        ],
        recommendedAction:
          'Define the key prerequisites needed to achieve this goal',
      }
    }
