  ...
  switch (nodeType) {
    case "GOAL":
      return [
        ...
        { key: "status", label: "Status", type: "select", options: STATUS_OPTIONS.GOAL },
    ...
    case "PREREQUISITE":
      return [
        { key: "title", label: "Title", type: "text" },
        { key: "description", label: "Description", type: "textarea" },
        { key: "status", label: "Status", type: "select", options: STATUS_OPTIONS.PREREQUISITE },
        { key: "confidenceScore", label: "Confidence (%)", type: "number" },
      ];
    case "ACTION":
      return [
        { key: "title", label: "Title", type: "text" },
        { key: "status", label: "Status", type: "select", options: STATUS_OPTIONS.ACTION },
    ...
    case "STAKEHOLDER":
      return [
        ...
        { key: "relationshipStrength", label: "Relationship Strength (%)", type: "number" },
...
export function NodeEditModal({ nodeId, nodeType, onClose, onSaved }: NodeEditModalProps) {
  ...
  async function handleDelete() {
    ...
    try {
      const res = await fetch(`${API_PATH[nodeType]}/${nodeId}`, { method: "DELETE" });
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-zinc-100">
          <div>
            ...
            <p className="text-xs text-zinc-400 mt-0.5">Click a field to edit</p>
          </div>
              ...
              <div key={field.key}>
                ...
                {field.type === "textarea" ? (
                  ...
                  <input
                    type="number"
                    min={0}
                    max={field.key.includes("Strength") || field.key.includes("confidence") ? 100 : undefined}
