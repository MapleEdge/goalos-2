...
function getTopBlocker(prereqs: Prerequisite[]): Prerequisite | null {
  const blocked = prereqs.filter((p) => p.status === "BLOCKED" || p.status === "NOT_STARTED");
  if (blocked.length === 0) return null;
  return blocked.sort((a, b) => a.confidenceScore - b.confidenceScore)[0];
}

function getNextAction(actions: Action[]): Action | null {
  const priorityOrder: Record<string, number> = { CRITICAL: 4, HIGH: 3, MEDIUM: 2, LOW: 1 };
  const pending = actions.filter((a) => a.status === "TODO" || a.status === "IN_PROGRESS");
  if (pending.length === 0) return null;
  return pending.sort((a, b) => (priorityOrder[b.priority] || 0) - (priorityOrder[a.priority] || 0))[0];
}
      ...
      <Card>
        ...
        {topBlocker && goal.status !== "COMPLETED" && (
          <div className="mt-3 flex items-center gap-2 rounded-lg bg-red-50 px-3 py-2">
            <svg width="14" height="14" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="2" className="text-red-500 flex-shrink-0">
              <circle cx="8" cy="8" r="6" />
              <path d="M8 5v3M8 10.5v.5" strokeLinecap="round" />
            </svg>
            <span className="text-xs text-red-700 font-medium">Blocked:</span>
            <span className="text-xs text-red-600 truncate">{topBlocker.title}</span>
            <span className="text-xs text-red-400 ml-auto flex-shrink-0">{topBlocker.confidenceScore}%</span>
          </div>
        )}
        ...
        {nextAction && goal.status !== "COMPLETED" && (
          <div className="mt-2 flex items-center gap-2 rounded-lg bg-zinc-50 px-3 py-2">
            <svg width="14" height="14" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="2" className="text-zinc-500 flex-shrink-0">
            ...
            <span className="text-xs text-zinc-700 truncate">{nextAction.title}</span>
              ...
              <div className="space-y-1.5">
                ...
                {goal.prerequisites.map((p) => (
                  <div key={p.id} className="flex items-center gap-2 text-xs">
                    ...
                    <span className="ml-auto text-zinc-400 tabular-nums">{p.confidenceScore}%</span>
                  </div>
                ))}
              </div>
            )}
