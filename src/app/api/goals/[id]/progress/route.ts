...
export async function POST(
  ...
  try {
    ...
    switch (action) {
      ...
      case "update_stakeholder": {
        if (body.stakeholderId) {
          ...
          await recordEvent("STAKEHOLDER", body.stakeholderId, "INTERACTION_UPDATED", {
            goalId: id,
            notes: body.notes,
          });
        }
      ...
      case "change_status": {
        await prisma.goal.update({
          ...
          data: {
            ...
            ...(body.status !== "COMPLETED" && goal.status === "COMPLETED" ? { completedAt: null } : {}),
          },
        });
    ...
    console.error("Progress API error:", message, err);
