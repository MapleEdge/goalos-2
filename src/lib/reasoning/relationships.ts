...
function suggestedCadenceDays(strength: number, linkedGoalCount: number): number {
...
function buildLinkedGoals(
  ...
  for (const rel of relationships) {
    ...
    if (rel.fromType === "STAKEHOLDER" && rel.fromId === stakeholderId && rel.toType === "GOAL") {
      goalId = rel.toId;
    } else if (rel.toType === "STAKEHOLDER" && rel.toId === stakeholderId && rel.fromType === "GOAL") {
      goalId = rel.fromId;
    }
    if (goalId) {
...
function buildTalkingPoints(
  ...
  if (linkedGoals.length > 0) {
    points.push(`Share an update on "${linkedGoals[0]}" and ask for their input`);
  }
  if (daysSinceContact !== null && daysSinceContact > 30) {
    points.push(`Acknowledge it's been a while (${daysSinceContact} days) and reconnect personally`);
  }
  ...
  if (s.notes) {
    points.push(`Follow up on your note: "${s.notes.slice(0, 80)}${s.notes.length > 80 ? "…" : ""}"`);
  }
  if (points.length === 0) {
    points.push(`Ask an open question to learn what they're focused on right now`);
  }
...
export function computeRelationshipHealth(
  ...
  const results = stakeholders.map((s) => {
    ...
    const cadence = suggestedCadenceDays(s.relationshipStrength, linkedGoals.length);
    ...
    const overdueDays =
      daysSinceContact !== null ? Math.max(0, daysSinceContact - cadence) : cadence;
    ...
    const stalenessRatio =
      daysSinceContact === null
        ? 1
        : Math.min(1, overdueDays / (cadence * 2));
    ...
    const recommendedStep = recommendStep(
      { daysSinceContact, overdueDays, relationshipStrength: s.relationshipStrength, linkedGoals },
      s.name
    );
